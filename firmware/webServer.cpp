...
extern "C" {
 ...
 uint16_t WebServer::HandleResponseEntry(const char *tag, char *pcInsert, int iInsertLen, uint16_t tagPart, uint16_t *nextPart, void *connectionState)
 {
    ...
    return ctx->pThis->HandleResponse(tag, pcInsert, iInsertLen, tagPart, nextPart, ctx);
 }
 ...
 uint16_t WebServer::HandleResponse(const char *tag, char *pcInsert, int iInsertLen, uint16_t tagPart, uint16_t *nextPart, const CgiContext *cgiContext)
 {
    ...
    if(!strcmp(tag, "result"))
    {
        ...
        return cgiContext->resultLength;
    }
...
uint16_t WebServer::HandleRequest(fs_file *file, const char *uri, int iNumParams, char **pcParam, char **pcValue, CgiContext *ctx)
{
    ...
    {
        return kvp->second(params, ctx->resultBuffer, sizeof(ctx->resultBuffer));
    }

    DBG_PUT("Ignoring unknown CGI\n");
    memcpy(ctx->resultBuffer, "false", 5);
    return 5;
}

void WebServer::AddRequestHandler(std::string url, CgiSubscribeResultFunc &&callback)
{
    _requestSubscriptions.emplace( std::move(url), std::move(callback));
...
{
    _responseSubscriptions.emplace(std::move(tag), std::move(callback));
}
